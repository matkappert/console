...
class _console {
  ...
  void printHelp() {
    ...
    printBox(true);
    v().pln("   OPTIONS").pln();
    v().p("   ").p("-? --help").p(":\t\t").pln("Displays a list of the available commands");
    v().p("   ").p("-i --info").p(":\t\t").pln("Displays firmware info");
    v().pln();
    for (size_t i = 0; i < _num; ++i) {
      v().p("   -").p(command -> shortcut).p(" --").p(command -> command).p("\t\t").p(command -> description).pln();
      ++command;
    }
    printBox(false);
  }
  ...
  void printInfo() {
    ...
    // v().p("   ").p("Build date").p(":\t\t").p(__DATE__).p(" - ").p(__TIMESTAMP__).p(" - ").pln(__TIME__);
